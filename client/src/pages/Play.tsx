...
import { buildAdaptiveRound, generateSmartReport, recordAnswer } from "@/game/adaptive";
import { questions } from "@/game/questions";
import { defaultProgress, loadProgress, saveProgress } from "@/game/storage";
import type { NexusProgress } from "@/game/types";
import { awardAdventureStage } from "@/game/progression";
...
export default function Play() {
  ...
  const [round, setRound] = useState(() => buildAdaptiveRound(questions, 3));
  const { isAuthenticated } = useAuth();
  const submitTournament = trpc.community.submitTournament.useMutation();
  const rewardWallet = trpc.economy.reward.useMutation();
  const question = useMemo(() => round[questionIndex] ?? questions[0], [questionIndex, round]);
  const total = 5;
  const stageId = typeof window === "undefined" ? 0 : Number(new URLSearchParams(window.location.search).get("stage") ?? 0);
  const adventureMode = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("mode") === "adventure" && stageId > 0;
  ...
  function startRound() {
    ...
    setStreak(0);
    setCompleted(false);
    setRound(buildAdaptiveRound(questions, adventureMode ? Math.min(4, stageId + 1) : 3));
  }
  ...
  function handleAnswer(option: number) {
    ...
    let nextProgress: NexusProgress = {
      ...current,
      xp: current.xp + (correct ? 32 : 8),
      totalSolved: current.totalSolved + 1,
      accuracy: Math.round(((current.accuracy / 100 * current.totalSolved + (correct ? 1 : 0)) / (current.totalSolved + 1)) * 100),
      bestStreak: Math.max(current.bestStreak, nextStreak),
      lastPlayedAt: new Date().toISOString(),
      updatedAt: Date.now(),
    };
    if (adventureMode && questionIndex >= total - 1) nextProgress = awardAdventureStage(nextProgress, stageId);
    setProgress(nextProgress);
    saveProgress(nextProgress);
    recordAnswer(question.category, correct, 30 - seconds);
    if (questionIndex >= total - 1 && isAuthenticated) {
      void submitTournament.mutateAsync({ score: nextScore });
      if (adventureMode && !current.completedStages.includes(stageId)) void rewardWallet.mutateAsync({ amount: stageId * 25 });
    }
    window.setTimeout(() => {
  ...
  if (!started) {
    return <NexusShell><section className="play-landing"><div className="play-landing__rail"><span className="eyebrow">{adventureMode ? "B / BOSS" : "A / 01"}</span><div className="vertical-word">{adventureMode ? "مواجهة الحارس" : "اختبار العقدة"}</div><div className="rail-calibration"><i /><i /><i /><i /><i /></div></div><div className="play-landing__content"><SectionKicker icon={Lightbulb}>{adventureMode ? `مواجهة المملكة ${String(stageId).padStart(2, "0")} · حارس معرفي` : `جلسة قصيرة · ${questions.length} عقد جاهزة`}</SectionKicker><h1>{adventureMode ? <>لا تهزم الحارس.<br /><em>افهمه.</em></> : <>لا تطارد الإجابة.<br /><em>اقترب منها.</em></>}</h1><p>{adventureMode ? "خمس عقد متكيفة تحسم المواجهة؛ الفوز يفتح مملكة جديدة ويمنحك ذهبًا وخبرة." : "جولة من خمس عقد، مؤقت يتكيف مع مستواك، وتفسير صغير يوضح لماذا كانت إجابتك منطقية أو تحتاج مراجعة."}</p><SignalTrace label="حالة المسار" value={adventureMode ? "الحارس حاضر · جاهز" : "تكيفي · جاهز"} /><div className="play-landing__stats"><div><strong>05</strong><span>عقد</span></div><div><strong>30<span>ث</span></strong><span>لكل عقدة</span></div><div><strong>x2</strong><span>أثر السلسلة</span></div></div><div className="hero-actions"><Button className="signal-button" onClick={startRound}><span>{adventureMode ? "ابدأ المواجهة" : "ابدأ الجولة"}</span><ArrowRight size={16} /></Button><Link href={adventureMode ? "/adventure" : "/"} className="text-link"><ChevronLeft size={15} /> العودة</Link></div></div><div className="play-landing__note"><span className="note-index">i</span><p>يختار المسار الفئة التي تحتاج تمرينًا أكثر، ويحفظ الأثر على هذا الجهاز.</p><div className="note-trace"><span /><span /><span /></div></div></section></NexusShell>;
  }

  if (completed) {
    const report = generateSmartReport();
    return <NexusShell><section className="result-panel"><div className="result-panel__marker"><Check size={25} /></div><SectionKicker icon={Flame}>الأثر اكتمل</SectionKicker><h1>جولة هادئة،<br /><em>نتيجة واضحة.</em></h1><div className="result-score"><span>النتيجة</span><strong>{score.toLocaleString("en-US")}</strong><small>نقطة أثر</small></div><div className="result-stats"><span>العقد المحلولة <b>{String(total).padStart(2, "0")} / {String(total).padStart(2, "0")}</b></span><span>أفضل سلسلة <b>{streak}</b></span><span>XP المكتسب <b>+{streak * 32 + 8}</b></span></div><div className="smart-report panel-surface"><SectionKicker icon={Lightbulb}>تقرير المسار</SectionKicker><strong>{report.headline}</strong><p>{report.focus}</p></div><div className="hero-actions"><Button className="signal-button" onClick={startRound}><RotateCcw size={16} /> جولة جديدة</Button><Link href="/" className="text-link"><ChevronLeft size={15} /> مركز Nexus</Link></div></section></NexusShell>;
  }

...
class ClassicLudoRules {
  ...
  static MatchState initialState({int humanPlayers = 1}) {
    ...
    return MatchState(players: players, currentPlayer: 0, phase: MatchPhase.rolling);
  }
  ...
  static List<String> legalPawnIds(MatchState state) {
    ...
    return state.activePlayer.pawns
        .where((Pawn pawn) => (pawn.progress == -1 && dice == 6) ||
            (pawn.progress >= 0 && pawn.progress + dice <= homeProgress))
        .map((Pawn pawn) => pawn.id)
        .toList();
  }

  static MatchState roll(MatchState state, int value) {
    if (state.phase != MatchPhase.rolling || value < 1 || value > 6) return state;
  ...
  static MatchState move(MatchState state, String pawnId) {
    if (!legalPawnIds(state).contains(pawnId)) return state;
    final int dice = state.dice!;
    ...
    final bool won = active.pawns.every((Pawn pawn) => pawn.progress == homeProgress);
    if (won) {

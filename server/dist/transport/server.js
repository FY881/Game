...
export function createTrustedServer(config, auth, rooms = new RoomService()) {
    const app = express();
...
function registerSocketHandlers(socket, rooms, io) {
    const user = socket.data.context.user;
    socket.emit('server:ready', { userId: user.id });
    socket.on('room:create', (raw, ack = () => undefined) => {
        try {
            const input = roomCreateSchema.parse(raw);
            const room = rooms.createRoom(user.id, input.maxPlayers, { mode: input.mode });
    ...
    socket.on('room:join', (raw, ack = () => undefined) => {
        try {
    ...
    socket.on('room:start', (raw, ack = () => undefined) => {
        try {
            const input = roomCodeSchema.parse(raw);
    ...
    socket.on('match:roll', (raw, ack = () => undefined) => {
        try {
    ...
    socket.on('match:move', (raw, ack = () => undefined) => {
        try {
...
function emitAck(ack, data) {
    ack({ ok: true, data });
}
function emitSocketFailure(socket, ack, error) {
